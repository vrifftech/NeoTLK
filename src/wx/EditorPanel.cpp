...
static_assert(neotlk::kNativeTlkSafetyApiVersion >= 1, "Update NeoShared: native TLK safety API required.");

namespace {
using neotlk::ui::TLKEditorPanel;
...
std::string displayTextForEntry(const neotlk::TalkString& entry) {
    ...
    std::string text;
    for (char c : entry.text) { if (c == '\0') text += "[NUL]"; else text += c; }
    std::string state;
    if (entry.flags & neotlk::SKIP_ENTRY) state += "Skipped by KotOR (0x8000): fallback table required.\n";
    if (!(entry.flags & neotlk::TEXT_PRESENT)) state += "Text-present flag is off; the following is stored data only.\n";
...
public:
    EntryDialog(wxWindow* parent, const wxString& title, neotlk::UInt32 strRef,
        ...
        if (source) {
            std::string displayed = source->text;
            const bool hasNul = displayed.find('\0') != std::string::npos;
            if (hasNul) {
                std::size_t pos = 0;
                while ((pos = displayed.find('\0', pos)) != std::string::npos) {
                    displayed.replace(pos, 1, "[NUL]");
                    pos += 5;
                }
            }
            text_->SetValue(wxui::toWx(displayed));
            // Never let a control's C-string truncation turn an ordinary edit
            // into accidental loss of the undisplayed original suffix.
            text_->SetEditable(!hasNul);
            ...
            if (soundMetadataEnabled_) {
                ...
                length_->SetValue(wxui::toWx(floatText(source->soundLength)));
            }
        } else {
        ...
        // Capture what the native widget displays (it may normalize CRLF).
        // Comparing to this snapshot preserves the original text on acceptance.
        initialText_ = text_->GetValue(); initialSound_ = resRef_->GetValue();
        initialVolume_ = volume_->GetValue(); initialPitch_ = pitch_->GetValue();
        initialLength_ = length_->GetValue();
        text_->SetName("entry.text"); resRef_->SetName("entry.sound");
        volume_->SetName("entry.volume"); pitch_->SetName("entry.pitch"); length_->SetName("entry.length");
        textFlag_->SetName("entry.textFlag"); soundFlag_->SetName("entry.soundFlag"); lengthFlag_->SetName("entry.lengthFlag");
        if (source && (source->flags & ~7u)) {
            auto* note = new wxStaticText(this, wxID_ANY,
                wxString::Format("Other flag bits are preserved: 0x%08X", static_cast<unsigned>(source->flags & ~7u)));
            root->Insert(1, note, 0, wxLEFT | wxRIGHT | wxBOTTOM, FromDIP(10));
        }
        if (source && source->text.find('\0') != std::string::npos) {
            auto* note = new wxStaticText(this, wxID_ANY,
                "Embedded NUL is shown as [NUL]. The original text is read-only until you choose the repair below; unchanged acceptance preserves all bytes.");
            note->Wrap(FromDIP(620));
            root->Insert(1, note, 0, wxLEFT | wxRIGHT | wxBOTTOM, FromDIP(10));
            auto* repair = new wxButton(this, wxID_ANY, "Use original text without NUL characters (keep suffix)");
            repair->SetName("entry.removeNul");
            root->Insert(2, repair, 0, wxLEFT | wxRIGHT | wxBOTTOM, FromDIP(10));
            repair->Bind(wxEVT_BUTTON, [this, repair](wxCommandEvent&) {
                std::string corrected = source_.text;
                corrected.erase(std::remove(corrected.begin(), corrected.end(), '\0'), corrected.end());
                forceTextChange_ = true; // Explicit action, even when the result looks unchanged.
                text_->SetValue(wxui::toWx(corrected));
                text_->SetEditable(true);
                repair->Enable(false);
            });
            text_->Bind(wxEVT_TEXT, [this, repair](wxCommandEvent& e) {
                // Do not replace a user's intervening edits with original text.
                repair->Enable(!forceTextChange_ && text_->GetValue() == initialText_);
                e.Skip();
            });
        }
        Layout();
    ...
    neotlk::TalkString entry() const {
        neotlk::EntryChanges changes;
        if (forceTextChange_ || text_->GetValue() != initialText_) changes.text = wxui::toStd(text_->GetValue());
        neotlk::UInt32 flags = 0;
        if (textFlag_->GetValue()) flags |= neotlk::TEXT_PRESENT;
        if (soundFlag_->GetValue()) flags |= neotlk::SND_PRESENT;
        if (lengthFlag_->GetValue()) flags |= neotlk::SNDLENGTH_PRESENT;
        changes.visibleFlags = flags;
        if (soundMetadataEnabled_) {
            if (resRef_->GetValue() != initialSound_) changes.sound = wxui::toStd(resRef_->GetValue());
            if (volume_->GetValue() != initialVolume_) changes.volume = parseUInt32(wxui::toStd(volume_->GetValue()), "volume");
            if (pitch_->GetValue() != initialPitch_) changes.pitch = parseUInt32(wxui::toStd(pitch_->GetValue()), "pitch");
            if (length_->GetValue() != initialLength_) changes.length = parseFloat(wxui::toStd(length_->GetValue()), "sound length");
        }
        return neotlk::applyEntryChanges(source_, changes, numericSoundIdMode_, soundMetadataEnabled_);
    }

private:
    neotlk::TalkString source_;
    bool forceTextChange_ = false;
    wxString initialText_, initialSound_, initialVolume_, initialPitch_, initialLength_;
...
class NeoTLKPanelImpl final : public neotlk::ui::TLKEditorPanel {
public:
    NeoTLKPanelImpl(wxWindow* parent, neomodules::Context context)
        : TLKEditorPanel(parent, std::move(context)) {
        buildMenus();
        buildLayout();
        createModuleStatusBar(1);
    ...
    bool openResource(neoshared::ResourceDocument input) override {
        ...
        auto loaded=neotlk::ResourceDocument::load(std::move(input));
        ensureDocumentTabForOpen(); auto& document=activeDocument();
        *document.table=std::move(loaded.model);
        ...
        document.viewState.resetForNewDocument(); if(filterText_)filterText_->ChangeValue("");
        updateLoadedState();showAllEntries();updateActiveTabTitle();
        setStatus("Archive snapshot: "+document.sourceDescription+". Save As creates a separate TLK.");
        grid_->SetFocus();return true;
    }
...
private:
    void checkDestination(const std::filesystem::path& path) const {
        ...
        if(ext!=".tlk") throw std::runtime_error("Save as a .tlk file, not as a source archive.");
    }
    ...
    enum : int {
        ...
        ID_FontIncrease,
        ID_FontDecrease,
        ID_FontReset, ID_ModuleExit, ID_ModuleAbout,
    };

    static constexpr int kRecentFileBaseId = wxID_HIGHEST + 28000;
    ...
    void buildMenus() {
        ...
        file->Append(ID_InterpretEncoding, "Interpret original text bytes as...");
        file->Append(ID_ConvertEncoding, "Convert native text encoding...");
        ...
        file->AppendSeparator();
        if (!context_.embedded) file->Append(ID_ModuleExit, "&Quit\tCtrl+Q");
        ...
        tools->Append(ID_ClearEntry, "Clear selected entry (keep StrRef)...");
        tools->Append(ID_DeleteSelected, "Remove row / compact table (advanced)...");
        tools->AppendSeparator();
        ...
        Bind(wxEVT_MENU, &NeoTLKPanelImpl::onNew, this, ID_New);
        Bind(wxEVT_MENU, &NeoTLKPanelImpl::onOpen, this, ID_Open);
        Bind(wxEVT_MENU, &NeoTLKPanelImpl::onOpenRecent, this, kRecentFileBaseId, kRecentFileBaseId + neosettings::kMaxRecentFiles - 1);
        Bind(wxEVT_MENU, &NeoTLKPanelImpl::onClearRecentFiles, this, kClearRecentFilesId);
        Bind(wxEVT_MENU, &NeoTLKPanelImpl::onSave, this, ID_Save);
        Bind(wxEVT_MENU, &NeoTLKPanelImpl::onSaveAs, this, ID_SaveAs);
        Bind(wxEVT_MENU, &NeoTLKPanelImpl::onCloseTab, this, ID_CloseTab);
        Bind(wxEVT_MENU, &NeoTLKPanelImpl::onCloseOtherTabs, this, ID_CloseOtherTabs);
        Bind(wxEVT_MENU, &NeoTLKPanelImpl::onNextTab, this, ID_NextTab);
        Bind(wxEVT_MENU, &NeoTLKPanelImpl::onPreviousTab, this, ID_PreviousTab);
        Bind(wxEVT_MENU, &NeoTLKPanelImpl::onSearch, this, ID_Search);
        Bind(wxEVT_MENU, &NeoTLKPanelImpl::onNewEntry, this, ID_NewEntry);
        Bind(wxEVT_MENU, &NeoTLKPanelImpl::onEditRow, this, ID_EditRow);
        Bind(wxEVT_MENU, &NeoTLKPanelImpl::onAppendFile, this, ID_AppendFile);
        Bind(wxEVT_MENU, &NeoTLKPanelImpl::onSetLanguage, this, ID_SetLanguage);
        Bind(wxEVT_MENU, &NeoTLKPanelImpl::onInterpretEncoding, this, ID_InterpretEncoding);
        Bind(wxEVT_MENU, &NeoTLKPanelImpl::onConvertEncoding, this, ID_ConvertEncoding);
        Bind(wxEVT_MENU, &NeoTLKPanelImpl::onClearEntry, this, ID_ClearEntry);
        Bind(wxEVT_MENU, &NeoTLKPanelImpl::onDeleteSelected, this, ID_DeleteSelected);
        Bind(wxEVT_MENU, &NeoTLKPanelImpl::onPadToStrRef, this, ID_PadToStrRef);
        Bind(wxEVT_MENU, &NeoTLKPanelImpl::onCopy, this, ID_Copy);
        Bind(wxEVT_MENU, &NeoTLKPanelImpl::onCut, this, ID_Cut);
        Bind(wxEVT_MENU, &NeoTLKPanelImpl::onPaste, this, ID_Paste);
        Bind(wxEVT_MENU, &NeoTLKPanelImpl::onSelectAll, this, ID_SelectAll);
        Bind(wxEVT_MENU, &NeoTLKPanelImpl::onFilterPrompt, this, ID_Filter);
        Bind(wxEVT_MENU, &NeoTLKPanelImpl::onClearFilter, this, ID_ClearFilter);
        Bind(wxEVT_MENU, &NeoTLKPanelImpl::onFilterSelectedColumn, this, ID_FilterColumn);
        Bind(wxEVT_MENU, &NeoTLKPanelImpl::onClearSelectedColumnFilter, this, ID_ClearColumnFilter);
        Bind(wxEVT_MENU, &NeoTLKPanelImpl::onClearAllFilters, this, ID_ClearAllFilters);
        Bind(wxEVT_MENU, &NeoTLKPanelImpl::onResetColumnOrder, this, ID_ResetColumnOrder);
        Bind(wxEVT_MENU, &NeoTLKPanelImpl::onResetRowOrder, this, ID_ResetRowOrder);
        ...
        Bind(wxEVT_MENU, &NeoTLKPanelImpl::onToggleDarkMode, this, ID_DarkMode);
        Bind(wxEVT_MENU, &NeoTLKPanelImpl::onIncreaseFontScale, this, ID_FontIncrease);
        Bind(wxEVT_MENU, &NeoTLKPanelImpl::onDecreaseFontScale, this, ID_FontDecrease);
        Bind(wxEVT_MENU, &NeoTLKPanelImpl::onResetFontScale, this, ID_FontReset);
        ...
        Bind(wxEVT_BUTTON, &NeoTLKPanelImpl::onNew, this, ID_New);
        Bind(wxEVT_BUTTON, &NeoTLKPanelImpl::onOpen, this, ID_Open);
        Bind(wxEVT_BUTTON, &NeoTLKPanelImpl::onSave, this, ID_Save);
        Bind(wxEVT_BUTTON, &NeoTLKPanelImpl::onSaveAs, this, ID_SaveAs);
        Bind(wxEVT_BUTTON, &NeoTLKPanelImpl::onClearFilter, this, ID_ClearFilter);
    }
    ...
    void buildLayout() {
        ...
        Bind(wxEVT_BUTTON, &NeoTLKPanelImpl::onShowInterval, this, ID_Show);
        documentTabs_->Bind(wxEVT_AUINOTEBOOK_PAGE_CHANGED, &NeoTLKPanelImpl::onDocumentTabChanged, this);
        documentTabs_->Bind(wxEVT_AUINOTEBOOK_PAGE_CLOSE, &NeoTLKPanelImpl::onDocumentTabCloseRequested, this);
        if (filterText_) filterText_->Bind(wxEVT_TEXT, &NeoTLKPanelImpl::onFilterText, this);
        ...
        grid_->Bind(wxEVT_LIST_COL_RIGHT_CLICK, &NeoTLKPanelImpl::onListColumnRightClick, this);
    ...
    void setStatus(const std::string& status) {
        ...
        setModuleStatusText( wxui::toWx(text));
    }
    ...
    void updateLoadedState() {
        ...
        {
            enableModuleCommand(ID_Save, loaded);
            enableModuleCommand(ID_SaveAs, loaded);
            enableModuleCommand(ID_SetLanguage, supportsLanguage);
            enableModuleCommand(ID_InterpretEncoding, supportsKotORPatcher);
            enableModuleCommand(ID_ConvertEncoding, supportsKotORPatcher);
            enableModuleCommand(ID_ClearEntry, hasEntries);
            enableModuleCommand(ID_Search, hasEntries);
            enableModuleCommand(ID_Filter, loaded);
            enableModuleCommand(ID_FilterColumn, loaded);
            enableModuleCommand(ID_ClearColumnFilter, loaded);
            enableModuleCommand(ID_ClearAllFilters, loaded);
            enableModuleCommand(ID_NewEntry, loaded);
            enableModuleCommand(ID_EditRow, hasEntries);
            enableModuleCommand(ID_DeleteSelected, hasEntries);
            enableModuleCommand(ID_PadToStrRef, canPad);
            enableModuleCommand(ID_AppendFile, loaded);
            enableModuleCommand(ID_ExportCsv, loaded);
            enableModuleCommand(ID_ExportTsv, loaded);
            enableModuleCommand(ID_ExportXml, loaded);
            enableModuleCommand(ID_ExportJson, loaded);
            enableModuleCommand(ID_ExportTslPatcher, supportsKotORPatcher);
            enableModuleCommand(ID_ExportHoloPatcher, supportsKotORPatcher);
        }

        if (filePath_ != nullptr) {
            const std::string path = loaded ? (table().hasSaveTarget() ? table().saveTargetFilename() : (!activeDocument().sourceDescription.empty() ? activeDocument().sourceDescription : table().filename())) : std::string();
    ...
    void loadFile(const std::string& file) {
        ...
        setModuleTitle(wxui::toWx(std::string("NeoTLK v") + neotlk::kVersion + " (TLK file editor) - " + file));
        ...
        const auto issues = table().nativeCompatibilityIssues();
        if (!issues.empty()) wxui::showMessage(this, "Native text needs review", issues.front() +
            "\n\nThe source is unchanged. Raw entry inspection and unchanged copying remain available.");
    }
    ...
    void onNew(wxCommandEvent&) {
        try {
            ...
            viewState().resetForNewDocument();
            if (filterText_) filterText_->ChangeValue("");
            setModuleTitle(wxui::toWx(std::string("NeoTLK v") + neotlk::kVersion + " (TLK file editor)"));
            start_->SetValue("0");
            stop_->SetValue("0");
            updateLoadedState();
            resetGridMessage("No entries present. Please use the Tools menu to add new entries.");
    ...
    void editSelectedEntry() {
        ...
        if (original.sameContent(replacement)) {
            setStatus("Entry unchanged; flags, original text and sound metadata retained.");
            return;
        }
        if (!table().supportsSoundMetadata()) {
    ...
    void onSetLanguage(wxCommandEvent&) {
        try {
            ...
            wxui::applyTheme(&dialog, darkMode_);
            if (dialog.ShowModal() != wxID_OK) return;
            if (dialog.language() != table().language() && !wxui::confirm(this, "Change language",
                "Change the language and validate the table-wide native encoding? Existing Unicode text is retained, not reinterpreted. An explicitly chosen code page stays selected.")) return;
            table().setLanguage(dialog.language());
            updateLoadedState(); refreshCurrentInterval();
    ...
    void onInterpretEncoding(wxCommandEvent&) {
        try {
            ensureLoaded();
            wxArrayString choices;
            choices.Add("Windows-1252 (Western)"); choices.Add("Windows-1250 (Central European / Polish)");
            choices.Add("UTF-8 (inspection only; convert before edited native save)");
            wxSingleChoiceDialog dialog(this,
                "Change how original bytes are displayed, not the bytes on disk. Requires an unchanged document.",
                "Text interpretation", choices);
            wxui::applyTheme(&dialog, darkMode_);
            if (dialog.ShowModal() != wxID_OK) return;
            const neotlk::TextEncoding encodings[] = {neotlk::TextEncoding::Windows1252, neotlk::TextEncoding::Windows1250, neotlk::TextEncoding::Utf8};
            if (dialog.GetSelection() < 0 || dialog.GetSelection() > 2) return;
            table().reinterpretTextEncoding(encodings[dialog.GetSelection()]);
            refreshCurrentInterval(); updateLoadedState();
            setStatus("Text interpretation: " + table().textEncodingSummary() + ". Original file bytes are unchanged.");
        } catch (const std::exception& ex) { wxui::showError(this, ex); }
    }

    void onConvertEncoding(wxCommandEvent&) {
        try {
            ensureLoaded();
            wxArrayString choices; choices.Add("Windows-1252 (Western)"); choices.Add("Windows-1250 (Central European / Polish)");
            wxSingleChoiceDialog dialog(this,
                "Retain the visible Unicode text and encode every native entry with one code page on Save. Unrepresentable text aborts the conversion.",
                "Native text encoding", choices);
            dialog.SetSelection(table().preferredTextEncoding() == neotlk::TextEncoding::Windows1250 ? 1 : 0);
            wxui::applyTheme(&dialog, darkMode_);
            if (dialog.ShowModal() != wxID_OK || dialog.GetSelection() < 0) return;
            table().setTextEncoding(dialog.GetSelection() == 1 ? neotlk::TextEncoding::Windows1250 : neotlk::TextEncoding::Windows1252);
            refreshCurrentInterval(); updateLoadedState();
            setStatus("Native output encoding: " + table().textEncodingSummary() + ". Save to commit.");
        } catch (const std::exception& ex) { wxui::showError(this, ex); }
    }

    void onClearEntry(wxCommandEvent&) {
        try {
            ensureLoaded(); const auto ref = selectedStrRef();
            if (!wxui::confirm(this, "Clear entry without renumbering",
                "Clear the selected entry's text, sound and sound settings? Its StrRef and all later IDs remain unchanged. Unexposed flag bits are preserved.")) return;
            table().clearEntry(ref);
            refreshCurrentIntervalOrSearchRow(ref); updateLoadedState();
            setStatus("Cleared entry " + std::to_string(ref) + "; no StrRefs were renumbered.");
        } catch (const std::exception& ex) { wxui::showError(this, ex); }
    }
...
TLKEditorPanel* createEditorPanel(wxWindow* parent, neomodules::Context context) {
    return new NeoTLKPanelImpl(parent, std::move(context));
}
}
